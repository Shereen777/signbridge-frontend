...
const app = {
  ...
  BACKEND_URL: '', // e.g., 'https://signbridge-backend.onrender.com'
  keepAliveTimer: null,
  ...
  init() {
    ...
    AuthService.onAuthChange(async (user) => {
      if (user) {
        ...
        // Personalization: preload user's top phrase videos
        this.preloadPersonalVideos(user.uid);
        // Start keep-alive ping so Render free tier doesn't sleep
        this.startKeepAlive();
        this.showLanding();
      } else {
        this.user = null;
  ...
  // ===== Personalization =====
  // Uses existing Firestore `PhraseService.getFrequent()` for top phrase lookup.
  // If BACKEND_URL is later configured, switches to the Flask backend automatically.
  async preloadPersonalVideos(uid) {
    let phrases = [];
    try {
      if (this.BACKEND_URL) {
        // Optional Flask path (only used if user deploys the backend to Render)
        const r = await fetch(`${this.BACKEND_URL}/api/personalize?uid=${encodeURIComponent(uid)}&limit=5`);
        if (r.ok) {
          const data = await r.json();
          phrases = data.phrases || [];
        }
      } else {
        // Default path: query Firestore directly via PhraseService
        const frequent = await PhraseService.getFrequent(uid, 5);
        phrases = frequent.map(p => p.phrase);
      }
    } catch { /* silent — non-critical */ }

    // Preload each phrase's video (if available)
    for (const phrase of phrases) {
      if (typeof preloadVideo === 'function') preloadVideo(phrase);
    }
  },

  startKeepAlive() {
    if (!this.BACKEND_URL || this.keepAliveTimer) return;
    this.keepAliveTimer = setInterval(() => {
      fetch(`${this.BACKEND_URL}/api/ping`).catch(() => {});
    }, 10 * 60 * 1000);
  },
