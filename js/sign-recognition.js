...
class SignRecognitionEngine {
  ...
  processResults(results) {
    ...
    if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
      ...
      // Motion-based detection takes precedence — catches J, Z, wave-HELLO
      // that pure shape rules can't see.
      const fingers = this.classifier.getFingers(lm);
      const motion = this.classifier.classifyMotion(fingers);

      // Determine what to emit: motion > high-conf word > letter
      let candidate, type;
      if (motion) {
        candidate = motion.word;
        type = 'word';
      } else if (result.word && result.word.confidence >= 0.8) {
